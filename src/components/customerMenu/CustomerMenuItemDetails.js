	...
	return (
		<div className="item-details-container">
			<div className="item-details item-details-alt-1">
				{item.badgeSrc ? (
					<img className="item-details-badge" src={item.badgeSrc} alt="" />
				) : (
					''
				)}
					...
					{item.allergies.map((allergy) => {
						return (
							<div className="item-details-allergy" key={allergy}>
								<img
									className="item-details-allergy-icon"
									src="#"
									alt=""
									title="allergy"
								/>
				...
				<div className="item-details-set-selection-container">
					{!item.setItem
						...
						: item.setMenus.map((menu) => {
								return (
									<div
										className="item-details-set-selection-category-section"
										key={menu.category}
									>
										<div className="item-details-set-selection-category">
											{menu.category}
										</div>
										<div className="item-details-set-item-list-container">
											{getItemsFromIds(menu.items).map((setItem) => {
												return (
													<>
														<input
															className="item-details-set-item-radio"
															type="radio"
															name={menu.category}
															id={setItem.name}
														/>
														<label
															className="item-details-set-item"
															key={setItem.name}
															htmlFor={setItem.name}
															id={setItem.name}
															title={menu.category}
															onClick={setSetItem}
														>
															<img
																className="item-details-set-item-image"
																src={setItem.imageSrc}
																alt={setItem.name}
															/>
															<div className="item-details-set-item-name">
																{setItem.name}
															</div>
														</label>
													</>
				...
				<button onClick={cancelItem} className="item-details-cancel-btn">
					cancel
				</button>
				<button onClick={addItem} className="item-details-add-btn">
					add to order
				</button>
			</div>
		</div>
