...
const CustomerMenuRecieptItem = ({ item, dispatch, showImg }) => {
	...
	const editSetItem = (idx) => {
		navigate(`customer-menu/confirm-item/${item.id}`, {
			state: { item: item, editSet: idx },
		})
	}

	return (
		<div className="menu-receipt-item">
			<button onClick={removeItem} className="menu-receipt-item-cancel">
				x
			</button>
			{showImg ? (
			...
			<div className="menu-receipt-item-amount-container">
				<button
					className="menu-receipt-item-amount-subtract"
					onClick={subtractItem}
				>
					-
				</button>
				<div className="menu-receipt-item-amount">{item.amount}</div>
				<button className="menu-receipt-item-amount-add" onClick={addItem}>
					+
				</button>
			</div>
			<span>x</span>
						...
						return (
							<div className="menu-receipt-item-set" key={index}>
								{contents.map((item) => {
									return (
										<div
											className="menu-receipt-item-set-item"
											key={item.category + index}
										>
											<div className="menu-receipt-item-set-item-category">
												{item.category}:
											</div>
											<div className="menu-receipt-item-set-item-name">
												{item.setItem}
											</div>
										</div>
									)
								...
								<button
									className="menu-receipt-item-set-change-btn"
									onClick={() => editSetItem(index)}
								>
									change
								</button>
							</div>
			...
			{item.amount > 1 ? (
				<div className="menu-receipt-item-subtotal">
					...
					<span className="menu-receipt-item-subtotal-value">
